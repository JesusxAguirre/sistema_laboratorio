    ...
    <app-layout title="Hematologia especial">
                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                       ...
                       <div class=" overflow-auto rounded-lg  hidden md:block">
                                ...
                                <tbody>
                                    <tr class="bg-blue-200" v-for="c in citologia.data" :key="c.id">
                                        <td class=" border border-gray-500 px-4 py-2 p-3 text-sm  text-gray-700  whitespace-nowrap">{{c.pacientes_id}}</td>
                                
                                        <td class=" border border-gray-500 px-4 py-2 p-3 text-sm  text-gray-700 whitespace-nowrap">{{ moment(c.fecha).format("DD-MM-YYYY") }}</td>
                                       
                                        <td class=" border border-gray-500 px-4 py-2 p-3 text-sm  text-gray-700 whitespace-nowrap">
                                            
                                            <i @click="modalClose=true; selectedDescarte= c" class="text-4xl fa-solid fa-trash-can text-red-400 hover:text-red-800"></i>                                            
                                            </td>
                            ...
                            <div v-for="c in citologia.data" :key="c" >
                                <div class="grid grid-cols-1 gap-4 md:hidden">
                                    <div class="mt-6 bg-blue-300 p-4 rounded-lg shadow">
                                        ...
                                        <div class="space-y-4">
                                            <div>
                                            <span class="p-1.5 rounded-lg text-xs font-medium uppercase tracking-wide bg-green-500">ID de paciente:</span> {{c.pacientes_id}}
                                            </div>
                                             <div>
                                            <span class="p-1.5 rounded-lg text-xs font-medium uppercase tracking-wide bg-green-500">Fecha:</span>  {{ moment(c.fecha).format("DD-MM-YYYY") }}
                                            </div>
                                        </div>
                                    <div class="mt-5 space-y-5 float-left">
                                    
                        <i @click="modalClose=true; selectedCitologia= c" class="text-4xl fa-solid fa-trash-can text-red-400 hover:text-red-800"></i>                                            
                                    </div>
                                </div>
                            </div>
                            
                        </div>  
                        <paginator class="mt-3" :paginator="citologia" />                     
                    </div>   
                </div>
            </div>
        </div>
       <jet-modal  :show="modalClose">
            ...
            <template v-slot:content >
                <p v-if="selectedCitologia">¿Esta seguro que desea eliminar este Descarte? </p>
            </template>
    ...
    export default defineComponent({
        ...
        data() {
            return {
                 moment: moment,
                  modalClose : false,
                selectedCitologia: Object,
            };
        },
        methods: {
           deleteCitologia(){

               Inertia.delete(route('citologiaurinaria.destroy', {citologiaurinarium: this.selectedCitologia}));
